...
class ProlicSpider(Spider):
    name = 'CA_Prolic_accountancy'
    base_url = "http://www2.dca.ca.gov/pls/wllpub/WLLQRYNA$LCEV2.QueryList?P_QTE_CODE=PA&P_QTE_PGM_CODE=0300&P_NAME={lname}&P_RECORD_SET_SIZE=50&Z_ACTION=Find"
    next_url = "http://www2.dca.ca.gov/pls/wllpub/WLLQRYNA$LCEV2.QueryList?P_QTE_CODE=PA&P_QTE_PGM_CODE=0300&P_NAME={lname}&P_RECORD_SET_SIZE=50&Z_START={start}&Z_ACTION=Next"
    ...
    def parse_info(self, response):
        ...
        prolic = CA_Prolic_accountancyItem()
        for tr in trs:
            tr = tr.text
            ...
            if 'Experience Completed:' in tr:
                prolic['experience_completed'] = tr.replace('Experience Completed:', '').strip().strip('Definition').strip()
            if 'Expiration Date:' in tr:
                prolic['expiration_date'] = re.sub(' {2,}', ' ', tr.replace('Expiration Date:', '').strip())
            if 'Issue Date:' in tr:
                prolic['issue_date'] = re.sub(' {2,}', ' ', tr.replace('Issue Date:', '').strip())
            if 'Address:' in tr:
            ...
            if 'Disciplinary Actions/License Restrictions:' in tr:
                prolic['actions'] = tr.replace('Disciplinary Actions/License Restrictions:', '').strip()
        yield prolic